...
- **轉折點偵測（2026-09-21）**：`src/swing.js` 的 `detectSwings(highs, lows, thresholdPct)`，
  ...
  **這個參數沒有標準答案**——設小轉折多而雜、設大只剩大波段，所以一定要把結果畫在
  ...
  同檔另有 `waveDirection()`（波：上升/下跌/盤整）與 `nearestLevels()`（支/阻：離現價
  最近的上下轉折點，不分頭底——書上前頭前底都可能當支撐或壓力）。
  **`waveDirection` 只用已確認轉折點下判斷**（未確認的還會動，拿它下結論方向會一直翻），
  但會另外回報未確認那段有沒有在創新高/破新低（`breaking`）。
  體檢卡會把所用轉折點的**日期**印出來——判斷可能基於兩個月前的轉折，不標日期會誤導
  （實例：2026-09-21 台積電被判「下跌波」但轉折點是 7 月的，同時正在創新高）。
  另有 `trendLines()`（切：上升切線＝最後兩底連線、下降切線＝最後兩頭連線，延伸到今天）。
  **只有方向對的切線才算數**（`valid`：底要愈墊愈高、頭要愈壓愈低），否則線畫得出來
  但不是書上說的切線；圖表也只畫 valid 的那條，避免版面變義大利麵。
  十字訣進度：**已做 波・均・量・強・切・支・阻（7/10）**；
  剩 型（型態辨識，易誤判，**不建議做**）、離（背離，需先寫 KD/RSI）。
