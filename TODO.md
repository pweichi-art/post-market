...
| 轉折點偵測（「波」的地基）| 🟡 09-21 | ZigZag 已完成並畫在 K 線上，**待使用者目視驗收**；尚未做波浪方向判定 |

測試：`node --test` 共 37 個全過（deduction 4 + chips 3 + prefs 7 + indicators 16 + swing 7）。
...
- [ ] **波：波浪方向判定**（驗收通過後做）：用轉折點判「頭頭高底底高＝上升波」等，
      加到體檢卡與掃描表。做多時應優先看上升波
- [ ] **支/阻**：轉折點已有，直接讀最近的前波高/低點即可（地基已完成，成本低）
- [ ] **切**：兩個轉折低點連線＝上升切線；突破下降切線是書上的轉折訊號
